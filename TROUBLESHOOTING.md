...
## 問題五：平行批量生成文件時 ZIP 目錄結構不一致

### 問題描述

使用平行處理為 76 個連結器批量生成文件時，每個子任務獨立運行並將 5 個 Markdown 文件打包成 ZIP。然而，解壓後發現各 ZIP 的內部目錄結構不一致：部分 ZIP 將文件放在根目錄，部分放在 `{connector-name}-mcp/` 子目錄，還有部分放在 `home/ubuntu/{connector-name}-docs/` 等深層路徑中。

### 根本原因

平行子任務在獨立的沙箱中運行，每個子任務自行決定文件的存放路徑和 ZIP 的打包方式。由於 prompt 中未嚴格規定 ZIP 內部的目錄結構，導致不同子任務產生了不同的路徑格式。此外，部分連結器名稱中包含空格或特殊字符（如 `tl;dv`、`Stripe API`），進一步增加了路徑不一致的可能性。

### 解決方案

撰寫了一個 Python 整合腳本 (`organize_files.py`)，該腳本：
1. 逐個解壓每個 ZIP 文件到臨時目錄
2. 遞迴搜索所有 `.md` 文件
3. 根據 JSON 結果文件中記錄的連結器名稱，將文件映射到統一的 `{connector-name}-mcp/` 目錄結構
4. 最終驗證所有 79 個連結器均擁有完整的 5 個文件

### 經驗教訓

在使用平行處理批量生成文件時，應在 prompt 中明確規定輸出文件的目錄結構和命名規範，以避免後續需要額外的整合步驟。
